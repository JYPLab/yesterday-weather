...
function parseSidoName(key: string): { sido: string; name: string } {
  for (const sido of SIDO_LIST) {
    if (key.startsWith(sido + ' ')) {
      return { sido, name: key.slice(sido.length + 1) };
    }
  }
  return { sido: '', name: key };
}

export const REGIONS = Object.entries(LOCATION_CODES).map(([key, code]) => {
  const { sido, name } = parseSidoName(key);
  return { sido, name, code };
});

export function findRegion(key: string) {
  const { sido, name } = parseSidoName(key);
  return REGIONS.find((r) => r.sido === sido && r.name === name);
}

export function searchRegions(query: string) {
  if (!query) return REGIONS;
  return REGIONS.filter(
    (r) => r.sido.includes(query) || r.name.includes(query)
  );
}

export function regionKey(region: { sido: string; name: string }): string {
